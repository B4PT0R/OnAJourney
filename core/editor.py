...
from core.utils import state, short_id, rerun, check_rerun
...
class Editor:
    ...
    _excluded=['parser','key','container','_code','code','event','submitted_code','on_submit','info_bar','menu_bar','kwargs','buttons']

    def __init__(self,code="",buttons=None,on_submit=None, info_bar=False, menu_bar=False, key=None,**kwargs):
        ...
        self.kwargs=kwargs
        self.container=None
        self.info_bar=InfoBar(self) if info_bar else None
        self.menu_bar=MenuBar(self) if menu_bar else None
        self.parser=editor_output_parser(self._code.get_value())
    ...
    def get_border_radius(self):
        first="0px 0px" if self.menu_bar is not None else "8px 8px"
        last="0px 0px" if self.info_bar is not None else "8px 8px"
        return f"{first} {last}"
    ...
    def get_params(self):
        ...
        border_radius=[]
        params=dict(
            lang=self.kwargs.get('lang','text'),
            key=self.key,
            ...
            props={ 
                ...
                "style":{
                    "borderRadius": self.get_border_radius()
                }
            }
        )
        params.update(self.kwargs)
        if self.info_bar:
            params.update(info=self.info_bar.get_dict())
        if self.menu_bar:
            params.update(menu=self.menu_bar.get_dict())
        return params
    ...
    def component(self):
        ...
        """
        output=code_editor(self._code.get_value(),**self.get_params())
        event,content=self.parser(self.get_output(output))
        self._code.from_ui(content)
        self.event=event
...
def editor(code="",lang="python",key=None,**kwargs):
    if not key:
        raise ValueError('No key provided. You must give a unique key to the editor widget.')
    if not key in state:
        state[key]=Editor(code=code,lang=lang,key=f"{key}_internal",**kwargs)
...
if __name__=='__main__':
    ...
    code=editor(code="This is a test",key='test',response_mode='blur')
    if code:
        st.code(code,language=state.test.lang)
    if code=="magic command":
        state.test.code="Hey! you typed a magic command"
        state.test.refresh()
